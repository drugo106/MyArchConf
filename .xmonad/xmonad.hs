...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    -- use command "xev" to get key info
    
    -- volume keys
    -- icons at /usr/share/notify-osd/icons/hicolor/scalable/status/
    --, ((0, 0xffc7), spawn "pactl set-sink-mute @DEFAULT_SINK@ toggle && notify-send -t 500 'Volume Muted' --icon=audio-volume-muted")
    --, ((0, 0xffc8), spawn "pactl set-sink-mute @DEFAULT_SINK@ false && pactl set-sink-volume @DEFAULT_SINK@ -10% && notify-send -t 500 $(awk -F\'[][]\' \'/dB/ { print $2 }\' <(amixer sget Master)) --icon=audio-volume-low")
    ...
    -- brightness
    , ((myModMask, 0xffc3), spawn "lux -a 10%")    
    , ((myModMask, 0xffc2), spawn "lux -s 10%")
    , ((myModMask, 0xffc4), spawn "lux -s 100%")

    -- keyboard_backlight
    --, ((0, 0xffc0), spawn "echo 0 | sudo tee /sys/class/leds/asus::kbd_backlight/brightness")
    --, ((0, 0xffc1), spawn "echo 1 | sudo tee /sys/class/leds/asus::kbd_backlight/brightness")

    -- quick screehot
    , ((0, 0xff61), spawn "scrot '/home/lorenzo/Immagini/%Y-%m-%d_$wx$h.png'")

    -- screenshot GUI
    , ((0, 0xff13), spawn "gnome-screenshot -i")
